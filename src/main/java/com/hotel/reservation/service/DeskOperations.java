...
public class DeskOperations {
    ...
    public DeskStay checkIn(Long roomId, String guest, LocalDate departure, BigDecimal rate) {
        ...
        DeskStay stay = stays.save(new DeskStay(room, guest.trim(), today, departure, total));
        audit.log(actor(), "CHECK_IN", "DeskStay", stay.getId(), "Walk-in check-in; room " + room.getNumber());
        return stay;
    }
    ...
    public void pay(Long stayId, BigDecimal amount, String key) {
        ...
        DeskStay stay = stays.lockById(stayId).orElseThrow(() -> new ResponseStatusException(NOT_FOUND));
        ...
        require(amount.compareTo(stay.getBalance()) <= 0, "Payment exceeds outstanding balance");
        DeskPayment payment = payments.save(new DeskPayment(stay, amount, key, actor()));
        stay.addPayment(amount);
        audit.log(actor(), "PAYMENT", "DeskPayment", payment.getId(), "Payment recorded for stay " + stayId);
    }
    ...
    public void checkOut(Long stayId) {
        DeskStay stay = stays.lockById(stayId).orElseThrow(() -> new ResponseStatusException(NOT_FOUND));
        require(!stay.isCheckedOut(), "Stay is already checked out");
        require(stay.getBalance().signum() == 0, "Settle balance before checkout");
        DeskRoom room = rooms.lockById(stay.getRoom().getId()).orElseThrow();
        stay.checkOut(); room.setOccupied(false);
        audit.log(actor(), "CHECK_OUT", "DeskStay", stay.getId(), "Checkout completed");
    }
    ...
    public DeskReservation reserve(Long roomId, String guest, LocalDate arrival, LocalDate departure, BigDecimal rate) {
        require(guest != null && !guest.isBlank() && guest.trim().length() <= 100,
                "Guest name is required (maximum 100 characters)");
        ...
        DeskRoom room = rooms.lockById(roomId).orElseThrow(() -> new ResponseStatusException(NOT_FOUND));
        require(!reservationConflict(roomId, arrival, departure), "Room has an overlapping confirmed reservation");
        ...
        require(!arrival.equals(LocalDate.now()) || !room.isOccupied(), "Room is currently occupied");
        DeskReservation result = reservations.save(new DeskReservation(room, guest.trim(), arrival, departure, rate));
        audit.log(actor(), "RESERVE", "DeskReservation", result.getId(), "Reservation created for room " + room.getNumber());
        return result;
    }
    ...
    public void cancelReservation(Long id) {
        ...
        require(reservation.getStatus() == DeskReservation.Status.CONFIRMED, "Only confirmed reservations can be cancelled");
        rooms.lockById(reservation.getRoom().getId()).orElseThrow();
        reservation.cancel();
        audit.log(actor(), "CANCEL", "DeskReservation", id, "Reservation cancelled");
    }
    ...
    public DeskStay arriveReservation(Long id, Long chosenRoomId) {
        ...
        audit.log(actor(), "CHECK_IN", "DeskStay", stay.getId(), "Check-in from reservation " + id);
        return stay;
    }
}